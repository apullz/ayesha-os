                ...
                sphagnum: {
                  ...
                  children: {
                    capillifolium: {
                      ...
                      gaelicName: "C\xF2innich-mh\xF2nadh (Red Moss)",
                ...
                polytrichum: {
                  ...
                  children: {
                    commune: {
                      ...
                      gaelicName: "C\xF2innich-r\xECoghail (Royal Moss)",
                ...
                herbertus: {
                  ...
                  children: {
                    borealis: {
                      ...
                      gaelicName: "Sn\xE0thad-fhraoich mh\xF2r (Great Heather Needle)",
                ...
                pteridium: {
                  ...
                  children: {
                    aquilinum: {
                      ...
                      gaelicName: "Raineach-mh\xF2r (Great Fern)",
                      geologicalEra: "Neogene, ~15 Mya",
...
function getNodeFromPath(root, pathSegments2) {
  let current = root;
  for (const segment of pathSegments2) {
...
function resolvePath(root, currentSegments, targetPath) {
...
function buildAsciiTree(node, indent = "", isLast = true) {
  let result = "";
    ...
    case "ls": {
      ...
      keys.forEach((key) => {
        ...
        if (child.rank === "species") {
          listOutput += `  \x1B[32m\u{1F4C4} ${key}.md\x1B[0m   (${child.commonName || "Native Species"})
`;
        } else {
          const rankColor = child.rank === "clade" ? "\x1B[36m" : child.rank === "class" ? "\x1B[35m" : child.rank === "family" ? "\x1B[34m" : "\x1B[33m";
          listOutput += `  ${rankColor}\u{1F4C1} ${key}/\x1B[0m   [${child.rank}] - ${child.commonName || ""}
`;
        }
      });
...
`;
    ...
    case "search":
    case "locate": {
      let recursiveSearch = function(node, currentPathArr) {
        const pathStr = "/" + currentPathArr.join("/");
        const isMatch = node.name.toLowerCase().includes(query) || node.commonName && node.commonName.toLowerCase().includes(query) || node.gaelicName && node.gaelicName.toLowerCase().includes(query) || node.description.toLowerCase().includes(query) || node.lore && node.lore.toLowerCase().includes(query);
        if (isMatch) {
          results.push({
            name: node.name,
            rank: node.rank,
...
async function runShell() {
  ...
  console.log("\x1B[1;32m          \u{1F332}  CALEDONIAN PHYLOGENETIC TERMINAL v1.2  \u{1F332}          \x1B[0m");
  console.log("\x1B[1;33m=================================================================\x1B[0m");
